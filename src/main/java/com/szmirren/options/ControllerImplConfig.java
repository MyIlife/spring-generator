...
public class ControllerImplConfig {
	...
	/** 生成模板的名字 */
	private String templateName = Constant.TEMPLATE_NAME_ROUTER_IMPL;
	/** 是否覆盖原文件 */
	 ...
	 * @param tableItem
	 */
	public ControllerImplConfig(ObservableList<TableAttributeKeyValue> item) {
		super();
	 ...
	 * @param tableItem
	 */
	public ControllerImplConfig(ObservableList<TableAttributeKeyValue> item, String templateName, boolean overrideFile) {
		super();
		if (item != null && !item.isEmpty()) {
			tableItem.addAll(item);
		}
	 ...
	 * 初始化默认数据
	 */
	public ControllerImplConfig initDefaultValue() {
