 ...
 *
 */
public class CControllerConfig {
	/** 设置的tableItem */
	private List<TableAttributeKeyValue> tableItem = new ArrayList<>();
	/** 生成模板的名字 */
	private String templateName = Constant.TEMPLATE_NAME_ROUTER_C;
	/** 是否覆盖原文件 */
	 ...
	 * @param tableItem
	 */
	public CControllerConfig(ObservableList<TableAttributeKeyValue> item) {
		super();
	 ...
	 * @param tableItem
	 */
	public CControllerConfig(ObservableList<TableAttributeKeyValue> item, String templateName, boolean overrideFile) {
		super();
		if (item != null && !item.isEmpty()) {
			tableItem.addAll(item);
		}
	 ...
	 * 初始化默认数据
	 */
	public CControllerConfig initDefaultValue() {
