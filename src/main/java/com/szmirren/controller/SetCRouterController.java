...
import com.szmirren.options.CControllerConfig;
 ...
 *
 */
public class SetCRouterController extends BaseController {
	 ...
	 */
	public void init() {
		LOG.debug("初始化SetCRouterController...");
		LOG.debug("初始化SetCRouterController->初始化属性...");
		// 添加右键删除属性
		...
		LOG.debug("初始化SetCRouterController->初始化模板文件名选择...");
		cboTemplate.getItems().addAll(indexController.getTemplateNameItems());
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_ROUTER_C)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_ROUTER_C);
		}
		LOG.debug("初始化SetCRouterController->初始化配置信息...");
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getCControllerConfig() == null) {
				loadConfig(getConfig());
			} else {
				loadConfig(indexController.getHistoryConfig().getCControllerConfig());
			}
		} else {
		...
		initLanguage();
		LOG.debug("初始化SetCRouterController-->成功!");
	}
	 ...
	 * @return
	 */
	public CControllerConfig getConfig() {
	 ...
	 * @return
	 */
	public CControllerConfig getConfig(String name) {
		LOG.debug("执行从数据库中获取配置文件...");
		try {
			CControllerConfig config = ConfigUtil.getCRouterConfig(name);
		...
		return new CControllerConfig().initDefaultValue();
	}
	 ...
	 * @return
	 */
	public CControllerConfig getThisConfig() {
		LOG.debug("执行获取当前页面配置文件...");
		CControllerConfig config = new CControllerConfig(tblPropertyValues, cboTemplate.getValue(), chkOverrideFile.isSelected());
	 ...
	 * @param config
	 */
	public void loadConfig(CControllerConfig config) {
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveCRouterConfig(getThisConfig(), configName);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setCControllerConfig(getThisConfig());
		getDialogStage().close();
	}
