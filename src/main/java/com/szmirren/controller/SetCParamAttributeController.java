 ...
 *
 */
public class SetCParamAttributeController extends BaseController {
	...
	public void init() {
		...
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_C_PARAM)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_C_PARAM);
		}
		...
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getCParamConfig() == null) {
				loadConfig(getConfig());
			} else {
				LOG.debug("加载数据...从上一次保存获取");
				loadConfig(indexController.getHistoryConfig().getCParamConfig());
	...
	public EntityConfig getConfig(String name) {
		...
		try {
			EntityConfig config = ConfigUtil.getCParamConfig(name);
		...
		return new EntityConfig(Constant.TEMPLATE_NAME_C_PARAM);
	}
	...
	public void loadTablePropertyValues(List<TableAttributeEntity> attr) {
		...
		for (TableAttributeEntity entity : attr) {
			if (entity.getTdColumnName().equals("id")) {
				continue;
			}
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveCParamConfig(getThisConfig(), configName);
	...
	public void onCancel(ActionEvent event) {
		...
		if (result) {
			indexController.getHistoryConfig().setCParamConfig(null);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setCParamConfig(getThisConfigHasProperty());
		getDialogStage().close();
	}
