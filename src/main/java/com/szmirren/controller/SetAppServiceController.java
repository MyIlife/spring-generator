...
import com.szmirren.options.AppServiceConfig;
 ...
 *
 */
public class SetAppServiceController extends BaseController {
	 ...
	 */
	public void init() {
		LOG.debug("初始化SetAppServiceConterller...");
		LOG.debug("初始化SetAppServiceConterller->初始化属性...");
		// 添加右键删除属性
		...
		LOG.debug("初始化SetAppServiceConterller->初始化模板文件名选择...");
		...
		LOG.debug("初始化SetAppServiceConterller->初始化配置信息...");
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getServiceConfig() == null) {
				loadConfig(getConfig());
			} else {
				loadConfig(indexController.getHistoryConfig().getAppServiceConfig());
			}
		} else {
		...
		initLanguage();
		LOG.debug("初始化SetAppServiceConterller-->成功!");
	}
	 ...
	 * @return
	 */
	public AppServiceConfig getConfig(String name) {
		LOG.debug("执行从数据库中获取配置文件...");
		try {
			AppServiceConfig config = ConfigUtil.getAppServiceConfig(name);
		...
		return new AppServiceConfig().initDefaultValue();
	}
	 ...
	 * @return
	 */
	public AppServiceConfig getThisConfig() {
		LOG.debug("执行获取当前页面配置文件...");
		AppServiceConfig config = new AppServiceConfig(tblPropertyValues, cboTemplate.getValue(), chkOverrideFile.isSelected());
	 ...
	 * @param config
	 */
	public void loadConfig(AppServiceConfig config) {
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveAppServiceConfig(getThisConfig(), configName);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setAppServiceConfig(getThisConfig());
		getDialogStage().close();
	}
