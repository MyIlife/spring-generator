 ...
 *
 */
public class SetCVOAttributeController extends BaseController {
	...
	public void init() {
		...
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_C_VO)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_C_VO);
		}
		...
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getCVOConfig() == null) {
				loadConfig(getConfig());
			} else {
				LOG.debug("加载数据...从上一次保存获取");
				loadConfig(indexController.getHistoryConfig().getCVOConfig());
	...
	public EntityConfig getConfig(String name) {
		...
		try {
			EntityConfig config = ConfigUtil.getCVOConfig(name);
		...
		return new EntityConfig(Constant.TEMPLATE_NAME_C_VO);
	}
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveCVOConfig(getThisConfig(), configName);
	...
	public void onCancel(ActionEvent event) {
		...
		if (result) {
			indexController.getHistoryConfig().setCVOConfig(null);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setCVOConfig(getThisConfigHasProperty());
		getDialogStage().close();
	}
