 ...
 *
 */
public class SetVOAttributeController extends BaseController {
	...
	public void init() {
		...
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_VO)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_VO);
		}
		...
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getVOConfig() == null) {
				loadConfig(getConfig());
			} else {
				LOG.debug("加载数据...从上一次保存获取");
				loadConfig(indexController.getHistoryConfig().getVOConfig());
	...
	public EntityConfig getConfig(String name) {
		...
		try {
			EntityConfig config = ConfigUtil.getVOConfig(name);
		...
		return new EntityConfig(Constant.TEMPLATE_NAME_VO);
	}
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveVOConfig(getThisConfig(), configName);
	...
	public void onCancel(ActionEvent event) {
		...
		if (result) {
			indexController.getHistoryConfig().setVOConfig(null);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setVOConfig(getThisConfigHasProperty());
		getDialogStage().close();
	}
