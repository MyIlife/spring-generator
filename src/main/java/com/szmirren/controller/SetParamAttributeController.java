 ...
 *
 */
public class SetParamAttributeController extends BaseController {
	...
	public void init() {
		...
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_PARAM)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_PARAM);
		}
		...
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getParamConfig() == null) {
				loadConfig(getConfig());
			} else {
				LOG.debug("加载数据...从上一次保存获取");
				loadConfig(indexController.getHistoryConfig().getParamConfig());
	...
	public EntityConfig getConfig(String name) {
		...
		try {
			EntityConfig config = ConfigUtil.getParamConfig(name);
		...
		return new EntityConfig(Constant.TEMPLATE_NAME_PARAM);
	}
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveParamConfig(getThisConfig(), configName);
	...
	public void onCancel(ActionEvent event) {
		...
		if (result) {
			indexController.getHistoryConfig().setParamConfig(null);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setParamConfig(getThisConfigHasProperty());
		getDialogStage().close();
	}
